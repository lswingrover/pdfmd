#!/usr/bin/env bash
# Activate the canonical git hooks for THIS clone (run once per clone; safe to re-run).
#
...
# the scotty repo's scripts/githooks/. dev-policy.md §2.
#
...
# Standardize on scripts/githooks. Some older repos use scripts/hooks; both are supported,
# and the first that exists wins — so a repo can migrate without a flag day.
HOOKS_REL=""
...
echo "core.hooksPath = $HOOKS_REL  (canonical hooks active for this clone + its worktrees)"
